...
/**
 * Seed realistic public students for Leaderboard and Compare features
 */
function seedSamplePublicStudents() {
  if (!dbCache) return;
  if (Object.keys(dbCache.users).length > 0) return;
  ...
  for (const s of sampleUsers) {
    ...
    const hash = bcrypt.hashSync(s.password, salt);
    ...
    dbCache.usernamesMap[s.username.toLowerCase().replace(/^@/, '')] = userId;
    dbCache.emailsMap[s.email.toLowerCase()] = userId;
  }
